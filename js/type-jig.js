...
TypeJig.prototype.reset = function() {
	...
	// document.getElementById('lesson').scroll(0, scrollOffset(this.display));
	document.getElementById('lesson').scrollTo(0, 0);
}
	...
	for(let i=0; i<answer.length; ++i) {
		...
		if(r.bottom > y + 0.001) {
			...
			if(endOfAnswer) {
				...
				// if(end > window.innerHeight && r.bottom > limit) document.getElementById('lesson').scrollBy(0, r.bottom - limit);
				let div = document.getElementById('lesson');
				if(end > div.getBoundingClientRect().height && r.bottom > limit) div.scrollBy(0, r.bottom - limit);
			}
		}
		y = r.bottom;
...
TypeJig.prototype.endExercise = function(seconds) {
	...
	let div = document.getElementById('lesson');
	var scroll = rect.bottom - div.getBoundingClientRect().height;
	// if(scroll > 0) setTimeout(function(){window.scrollBy(0, scroll)}, 2);
	if(scroll > 0) setTimeout(function(){document.getElementById('lesson').scrollBy(0, scroll)}, 2);
